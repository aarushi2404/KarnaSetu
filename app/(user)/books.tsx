...
export default function BooksBoard() {
  ...
  const load = async () => {
    ...
    if (profileError) {
      console.log("Profiles load error:", profileError.message);
      setItems(books);
      return;
    }
  ...
  const request = async (item: BookDisplay) => {
    ...
    if (error) {
      return Alert.alert("Could not request", error.message);
    }
  ...
  const statusColor = (status: BookDonation["status"]) =>
    status === "available"
      ? "#15803D"
      : status === "reserved"
      ? "#B45309"
    ...
    if (!person) {
      return (
        <View style={styles.personBox}>
          ...
          <Text style={styles.personName}>Profile unavailable</Text>
        </View>
      );
    }
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Books & Items Board</Text>

        <TouchableOpacity
          style={styles.addBtn}
          onPress={() => setModalOpen(true)}
        >
          <Text style={styles.addBtnText}>+ Give an item</Text>
        </TouchableOpacity>
      </View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          const isRequester = item.requested_by === profile?.id;
          ...
          return (
            <View style={styles.card}>
              ...
              {isRequester && item.status === "reserved" ? (
                <Text style={styles.handoverText}>
                  Coordinate the handover directly with the owner.
                </Text>
              ) : null}
            </View>
          );
